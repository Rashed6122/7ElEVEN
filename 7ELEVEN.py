...
def tsp_subset_iterative(graph, start, end, shopping_list):
    n = len(graph)
    ...
    return round(optimal_distance, 2), optimal_route
...
input =  ['Accessories/Headwear', 'Novels/Magazzine', 'Energy Drinks/Wine/Beer']
shopping_list= []
for i in input:
...
shortest_route_distance, shortest_route = tsp_subset_iterative(graph, start_node, end_node, shopping_list)
output = []
for j in shortest_route:
    output.append(marketList[j])
print(output)